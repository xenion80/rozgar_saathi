...
class OpportunityServiceTest {
    ...
    void recommendedOpportunitiesAreSortedByMatchScoreDescendingAndOnlyOpenAreConsidered() {
        ...
        OpportunitySkillRepository opportunitySkillRepository = mock(OpportunitySkillRepository.class);
        SkillRepository skillRepository = mock(SkillRepository.class);
        OpportunityMatchingService matchingService = mock(OpportunityMatchingService.class);

        Opportunity backend = opportunity(1L, "Backend Developer Intern", "TechNova");
        Opportunity frontend = opportunity(2L, "Frontend Developer Intern", "CodeCraft");
        Opportunity dataAnalyst = opportunity(3L, "Data Analyst Intern", "Insight");

        when(opportunityRepository.findByStatus(OpportunityStatus.OPEN))
                .thenReturn(List.of(frontend, dataAnalyst, backend)); // deliberately unsorted
        ...
        when(matchingService.match(any(), any())).thenAnswer(invocation -> {
            ...
            return new MatchResponse(opp.getId(), opp.getTitle(), score,
                    List.of(), List.of(), score >= 60, List.of());
        });

        OpportunityService service = new OpportunityService(
                opportunityRepository, opportunitySkillRepository, skillRepository, matchingService);

        List<OpportunityMatchSummary> recommended = service.recommended(student);

        assertEquals(3, recommended.size());
        assertEquals(85, recommended.get(0).getMatchScore());
        assertEquals("Backend Developer Intern", recommended.get(0).getTitle());
        assertEquals(70, recommended.get(1).getMatchScore());
        assertEquals(45, recommended.get(2).getMatchScore());

        verify(opportunityRepository).findByStatus(OpportunityStatus.OPEN);
    }

    private Opportunity opportunity(Long id, String title, String company) {
