  ...
  return (
    <div className="space-y-6 ">
      <div>
        ...
        <p className="text-slate-500 mt-2 dark:text-slate-300">{description}</p>
      </div>

      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500 dark:text-white" />
          <Input
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {data?.content.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-300">#{user.id}</td>
                    <td className="px-6 py-4 text-slate-900 dark:text-slate-300">{user.name}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-300">{user.email}</td>
                      ...
                      <Badge variant="outline" className={
                        user.role === "STUDENT" ? "text-emerald-700 bg-emerald-50 border-emerald-200" :
                          user.role === "RECRUITER" ? "text-orange-700 bg-orange-50 border-orange-200" :
                            "text-slate-700 dark:text-slate-900 bg-slate-100 dark:bg-white! border-slate-200 dark:border-slate-700"
                      }>
                        {user.role}
                      </Badge>
                    </td>
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
            <span className="text-sm text-slate-500 dark:text-slate-400">
              Showing <span className="font-medium text-slate-900 dark:text-white">{data.number * data.size + 1}</span> to <span className="font-medium text-slate-900 dark:text-white">{Math.min((data.number + 1) * data.size, data.totalElements)}</span> of <span className="font-medium text-slate-900 dark:text-white">{data.totalElements}</span> results
            </span>
